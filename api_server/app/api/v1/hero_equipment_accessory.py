...
@api_root.resource('/v1/heroes/<int:hero_id>/equipments/accessory/put-on')
class HeroEquipmentAccessoryPutOn(Resource):
    ...
    def get(self, request_user, hero_id):
        ...
        if hero.equipment_id_accessory is not None:
            prev_equipment = EquipmentModel.query.\
                filter(EquipmentModel.id == hero.equipment_id_accessory).\
                first()
            prev_equipment.hero_id = None
            hero.equipment_id_accessory = None

        equipment_metadata = MetadataEquipmentModel.query.\
            filter(MetadataEquipmentModel.id == equipment.equipment_metadata_id).\
            first()
        if equipment_metadata.type != 'accessory':
            abort(400)

        hero.equipment_id_accessory = equipment.id
...
@api_root.resource('/v1/heroes/<int:hero_id>/equipments/accessory/put-off')
class HeroEquipmentAccessoryPutOff(Resource):
    @access_token_required
    def get(self, request_user, hero_id):
        hero = HeroModel.query.\
            filter(HeroModel.id == hero_id).\
        ...
        if hero.equipment_id_accessory is None:
            abort(400)
